...
import com.hsun.ywork.common.utils.DocConverter;
...
import com.hsun.ywork.modules.sys.security.SystemAuthorizingRealm;
import com.hsun.ywork.modules.sys.utils.LogUtils;
...
public class SystemService extends BaseService implements InitializingBean {
	 ...
	 * @return
	 */
	public List<User> findUserNoDsf(User user){
		List<User> list = userDao.findListNoDsf(user);
		return list;
	}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public static boolean printKeyLoadMessage(){
		...
		sb.append("\r\n======================================================================\r\n");
		sb.append("\r\n    欢迎使用 "+ Global.getConfig("productName")+"  - Powered By GeCoder\r\n");
		sb.append("\r\n======================================================================\r\n");
	...
	public void registerUser(User user){
		...
		company.setType("1");//设置机构类型为：公司
		company.setGrade("1");//设置机构级别为：一级
		company.setUseable("1");
		officeService.save(company);
		Office dept = new Office();
		dept.setParent(company);
		dept.setParentIds(company.getId());
		dept.setName("默认部门");
		dept.setType("2");//设置机构类型为：部门
		dept.setGrade("2");//设置机构级别为：二级
		dept.setUseable("1");
		officeService.save(dept);
		...
		user.setOffice(dept);
		user.setCategory("1");//设置用户分类为：企业注册用户
		user.setUserType("1");//设置用户类型为：系统管理（前台有进入控制台下拉菜单）
		user.setEmail(user.getLoginName());
//		user.setName(user.getLoginName());

		// 需提前在系统后台设置好企业注册用户默认角色及权限，在这里设置好用户角色，注册完毕登录后，企业注册用户可在自己公司内进行员工维护
		// 赋予企业注册用户以下权限
		List<Role> roleList = Lists.newArrayList();
		roleList.add(this.getRoleByEnname("ROLE_CUS_ADMIN"));	// 管理员
		roleList.add(this.getRoleByEnname("ROLE_CUS_SUBADMIN"));	// 子管理员
		roleList.add(this.getRoleByEnname("ROLE_CUS_MANAGER"));	// 主管
		roleList.add(this.getRoleByEnname("ROLE_CUS_NORMAL"));	// 员工
		roleList.add(this.getRoleByEnname("ROLE_CUS_JGLD"));	// 监管领导
