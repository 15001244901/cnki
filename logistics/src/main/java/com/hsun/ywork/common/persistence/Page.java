...
public class Page<T> {
	...
	public String toString() {
		...
		//pageStyle = 2 代表前台分页，直接返回前台分页的html代码
		if(pageStyle == 2){
			return getFrontPageHtml();
		}

		//以下代码处理后台管理页面分页的html代码构建
...
//		sb.insert(0,"<div class=\"page\">\n").append("</div>\n");
	...
	public String getFrontPageHtml(){
		...
		if (pageNo == first) {// 如果是首页
			sb.append("<a class=\"next prev disabled\" href=\"javascript:\">上一页</a>\n");
		} else {
			sb.append("<a class=\"next prev\" href=\"javascript:\" onclick=\""+funcName+"("+prev+","+pageSize+",'"+funcParam+"');\">上一页</a>\n");
		}
		...
		for (int i = begin; i <= end; i++) {
			if (i == pageNo) {
				sb.append("<a class=\"curr\" href=\"javascript:\">" + (i + 1 - first)
						+ "</a>\n");
			} else {
				sb.append("<a class=\"num\" href=\"javascript:\" onclick=\""+funcName+"("+i+","+pageSize+",'"+funcParam+"');\">"
						+ (i + 1 - first) + "</a>\n");
			}
		}
		...
		if (pageNo == last) {
			...
			sb.append("<a class=\"next\" href=\"javascript:\" onclick=\""+funcName+"("+next+","+pageSize+",'"+funcParam+"');\">"
					+ "下一页</a>\n");
		}
		return sb.toString();
	}
...
//	}
	...
	public long getTotalPageCount() {
		return this.pageSize != 0 && this.count % this.pageSize == 0?this.count / this.pageSize:(this.pageSize != 0 && this.count % this.pageSize != 0?this.count / this.pageSize + 1:0);
	}
}
