...
function addPraise(targetId,type,obj){
	if(isLogin()){
		$.ajax({
			url:baselocation +urlpath+"/qa/praise/ajax/add",
			data:{
...
function getCourseLearnedUser(courseId){
	$.ajax({
		type : "POST",
		dataType : "json",
		url:ctxRoot+"/course/couserStudyHistory/ajax/courseLearnedUser/"+courseId,
		cache : true,
		async : false,
		success : function(result) {
			if(result.success==true){
					...
					var useImg="";
	 				...
	 				for(var i=0;i<resultList.length;i++){
	 					...
	 					if(useImg==null || $.trim(useImg)==''){
	 						useImg = ctxStatic+'/hsun/front/qa/img/avatar-boy.gif';
	 					}else{
