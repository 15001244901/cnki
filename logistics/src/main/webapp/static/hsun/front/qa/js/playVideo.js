...
function browserRedirect() {
	...
	if (bIsIphoneOs || bIsMidp || bIsUc7 || bIsUc || bIsAndroid || bIsCE || bIsWM) { // 移动端环境下效果
		...
		var wW = parseInt(document.documentElement.clientWidth, 10);
		// $("#p-h-box").css("height", Math.ceil((9*wW)/16)+100);
		$("#p-h-r-cont").css({"height":wH - 363,'backgroundColor':'#fff'});
		$(".p-h-video").css({"height":Math.ceil((9*wW)/16) + 28});
		$('#videoPlay').css({'height':Math.ceil((9*wW)/16)+4,'paddingTop':14});

		// $("#o-c-btn").click(function() {
		// 	$('.p-h-r-title,#p-h-r-cont').show();
        //
		// 	var _this = $(this);
		// 	if (!_this.hasClass("open-o-c")) {
		// 		$(".p-h-r-ele").animate({
...
function getPlayerHtml(kpointId,name,obj) {
	...
	$.ajax({
		url : "" + ctxRoot + "/course/ajax/getKopintHtml",
		data : {
			"kpointId" : kpointId,
			"courseId" : otherId
		},
...
function favorites(courseId,obj) {
	$.ajax({
		url : ctxRoot + '/course/createfavorites/' + courseId,
		type : 'post',
		dataType : 'json',
		success : function(result) {
 ...
 function addNotest() {
 	...
 	if (KindEditor.get("#notesContextId").isEmpty()) {
 		...
 		$.ajax({
 			url : ctxRoot + "/course/courseNote/ajax/addnote",
 			type : 'post',
 			dataType : 'json',
 			data : {
...
function queryNote() {
	$.ajax({
		url : ctxRoot + "/course/courseNote/ajax/querynote",
		type : "post",
		dataType : "json",
		data : {
			'kpointId' : currentKpointId,
...
function addPlayTimes(courseId, kpointId) {
	$.ajax({
		url : ctxRoot + "/course/couserStudyHistory/ajax/playertimes",
		data : {
			"kpointId" : kpointId,
			"courseId" : courseId
		},
