...
function addaKpoint(courseId){
	...
	$.ajax({
		url:ctx+'/course/kpoint/addkpoint',
		type:'post',
		dataType:'json',
		data:{
...
function initUpdateKpoint(treeId, treeNode){
	...
	$.ajax({
		url:ctx+'/course/kpoint/getkpoint/'+treeNode.kpointId,
		type:'post',
		dataType:'json',
		success:function(result){
			...
			//设置HTML内容
            contentCkeditor.setData(obj.content);
			...
			$('#teacher').val(obj.teacherId).trigger("change");
...
function updateKpoint(){
    $("#content").val(contentCkeditor.getData());
    // console.log(CKEDITOR.instances.cke_content.getData());
    console.log(contentCkeditor.getData());
	...
	if($("#courseKpointKpointType").val()==1) {//章节专属验证
		...
		if (fileType == 'TXT') {
            ...
            if (isEmpty(contentCkeditor.getData())) {
            	alert("文本内容必填！");
            	return;
            }
			params += "courseKpoint.content=" + contentCkeditor.getData() + "&";
	...
	var teacherId=$("#teacher").val()||'0';
	params+="courseKpoint.teacherId="+teacherId+"&";
	$.ajax({
		url:ctx+'/course/kpoint/updateKpoint',
		type:'post',
		dataType:'json',
		data:params,
		success:function(result){
...
function deleteKpoint(treeId, treeNode){
	...
	$.ajax({
		url:ctx+'/course/kpoint/deletekpoint/'+ids,
		type:'post',
		dataType:'json',
		async:false,
		success:function(result){
...
function updateKpointParentId(treeId, treeNodes, targetNode, moveType){
	...
	var is = true;
	$.ajax({
		url:ctx+'/course/kpoint/updateparentid/'+parentId+'/'+kpointId,
		type:'post',
		async:false,
		dataType:'json',
		success:function(result){
...
function getKpointType(parentId){
	var isTrue=true;
	$.ajax({
		url:ctx+'/course/kpoint/getkpoint/'+parentId,
		type:'post',
		dataType:'json',
		async:false,
		success:function(result){
...
function on_spark_selected_file(filename) {
	...
	$.ajax({
		url:ctx+'/course/ajax/kpoint/ccVideoTHQSData',
