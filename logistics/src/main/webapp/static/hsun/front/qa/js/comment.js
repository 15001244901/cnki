...
function queryComment() {
	ajaxPage(ctxRoot + "/comment/ajax/query", "&otherId=" + otherId + "&type=" + type+ "&order=commentId", 1, commentCallBack);
}
//查询评论
function queryWonderfulComment() {
	ajaxPage(ctxRoot + "/comment/ajax/query", "&otherId=" + otherId + "&type=" + type+ "&order=praiseCount", 1, commentCallBack);
}
...
function queryCommentReply(pCommentId) {
	$.ajax({
		url : ctxRoot + '/comment/ajax/commentreply',
		data : {
			"otherId" : otherId,
...
function addCommentReply(commentId) {
	...
	$.ajax({
		url : ctxRoot + '/comment/ajax/addcomment',
		type : 'post',
		data : {
...
function queryCommentReplyPage(pCommentId) {
	...
	$.ajax({
		url : ctxRoot + '/comment/ajax/commentreplypage',
		data : {
			"otherId" : otherId,
			"pCommentId" : pCommentId
		},
