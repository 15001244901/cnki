	...
	init: function() {
		...
		$.ajax({
			...
			success: function(t) {
				...
				if(success) {
					var datas = t.data;
					subjecthtml.push('<option value="">选择科目</option>');
				...
				$.ajax({
					...
					success: function(t) {
						...
						if(success) {
							var datas = t.data;
							questionhtml.push('<option value="">选择题型</option>');
							for(i = 0; i < datas.length; i++) {
						...
						$.ajax({
							...
							success: function(t) {
								...
								if(success) {
									var datas = t.data;
									levelhtml.push('<option value="">选择难度</option>');
									for(i = 0; i < datas.length; i++) {
										...
										if(success) {
											...
											if(typeof(datas)!="undefined")
											{
												for(i = 0; i < datas.length; i++) {
													...
													contenthtml.push('    <dt>');
													contenthtml.push('      <span class="section_title">');
													contenthtml.push('      段落设定 :');
													contenthtml.push('      <select id="p_subject" class="p_sel" name="p_subjects">');
							
													var datasubject = subject.data;
													contenthtml.push('<option value="">选择科目</option>');
													for(j = 0; j < datasubject.length; j++) 
													{
														if(datas[i].subject==datasubject[j].value)
														{
															contenthtml.push('<option selected="selected" value="' + datasubject[j].value + '">' + datasubject[j].label + '</option>');
														}
														else
														{
															contenthtml.push('<option value="' + datasubject[j].value + '">' + datasubject[j].label + '</option>');
														}
														
													}
													//alert(datasubject)
													//contenthtml.push(paperEditNormal.searchSubject());
													contenthtml.push('      </select>');
													contenthtml.push('      <select id="p_types" class="p_sel" name="p_qtypes">');
													
													var datatypes = types.data;
													contenthtml.push('<option value="">选择题型</option>');
													for(j = 0; j < datatypes.length; j++) 
													{
														if(datas[i].rtype==datatypes[j].value)
														{
															contenthtml.push('<option selected="selected" value="' + datatypes[j].value + '">' + datatypes[j].label + '</option>');
														}
														else
														{
															contenthtml.push('<option value="' + datatypes[j].value + '">' + datatypes[j].label + '</option>');
														}
														
													}
													
													contenthtml.push('      </select>');
													contenthtml.push('      <select id="p_levels" class="p_sel_4" name="p_levels">');
													
													var datalevel = level.data;
													contenthtml.push('<option value="">选择难度</option>');
													for(j = 0; j < datalevel.length; j++) 
													{
														if(datas[i].rlevel==datalevel[j].value)
														{
															contenthtml.push('<option selected="selected" value="' + datalevel[j].value + '">' + datalevel[j].label + '</option>');
														}
														else
														{
															contenthtml.push('<option value="' + datalevel[j].value + '">' + datalevel[j].label + '</option>');
														}
														
													}
													
													contenthtml.push('      </select>');
													contenthtml.push('      试题数量:');
													contenthtml.push('      <input type="text" class="p_nums" name="p_qnums" value="'+datas[i].rnum+'" />');
													contenthtml.push('      每题分值:');
													contenthtml.push('      <input type="text" class="p_scores" name="p_scores" value="'+datas[i].rscore+'"/>');
													contenthtml.push('      </span>');
													contenthtml.push('    </dt>');
...
var tmPaper = {
	uiInit: function() {
		...
		$(".tm_adm_questionlist").bind('sortstop', function(event, ui) {
			...
			questionUL.find("input[name^='p_question_cnts_']").attr("name", "p_question_cnts_" + section_id);
			 ...
			 //拖动后将当前项分值设置为段落设定的分值
            var sectionScore = ui["item"].closest(".tm_adm_paper_body_section_dl").find('input[name="p_scores"]').val();
            //alert(sectionScore)
            ui["item"].find('.txtScore').val(sectionScore);

            //拖动后将段落试题数量更新为实际试题数量
            var itemNums = ui["item"].closest(".tm_adm_questionlist").children().length;
            ui["item"].closest(".tm_adm_paper_body_section_dl").find('input[name="p_qnums"]').val(itemNums);
		});
	...
	addSection: function() {
		...
		html.push('      <span class="section_title">');
		html.push('      段落设定 :');
		html.push('      <select id="p_subject" class="p_sel" name="p_subjects">');

		var datasubject = subject.data;
		html.push('<option value="">选择科目</option>');
		for(j = 0; j < datasubject.length; j++) 
		{
			html.push('<option value="' + datasubject[j].value + '">' + datasubject[j].label + '</option>');
		}
		html.push('      </select>');
		html.push('      <select id="p_types" class="p_sel" name="p_qtypes">');
		
		var datatypes = types.data;
		html.push('<option value="">选择题型</option>');
		for(j = 0; j < datatypes.length; j++) 
		{
			html.push('<option value="' + datatypes[j].value + '">' + datatypes[j].label + '</option>');
		}
		
		html.push('      </select>');
		html.push('      <select id="p_levels" class="p_sel_4" name="p_levels">');
		
		var datalevel = level.data;
		html.push('<option value="">选择难度</option>');
		for(j = 0; j < datalevel.length; j++) 
		{
			html.push('<option value="' + datalevel[j].value + '">' + datalevel[j].label + '</option>');
		}
		
		html.push('      </select>');
		html.push('      试题数量:');
		html.push('      <input type="text" class="p_nums" name="p_qnums" value="1" />');
		html.push('      每题分值:');
		html.push('      <input type="text" class="p_scores" name="p_scores" value="0"/>');
		html.push('      </span>');
		html.push('    </dt>');
		
		
		html.push('  <dd>');
		html.push('     <ul class="tm_adm_questionlist1" data-sectionid="' + var_sectionId + '"></ul>');
		html.push('  </dd>');
		html.push(' </dl>');
		html.push('</li>');
	...
	removeItem: function(obj) {
		//删除后将段落试题数量更新为实际试题数量
        var itemNums = $(obj).closest(".tm_adm_questionlist").children().length-1;
        $(obj).closest(".tm_adm_paper_body_section_dl").find('input[name="p_qnums"]').val(itemNums);

		
		
		var pobj = $(obj).parent().parent().parent().parent().parent();
		if(pobj.is("li")) {
			pobj.remove();
		} else {
			pobj.parent().remove();
