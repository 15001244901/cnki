...
var inTheTest = {
	...
	init: function () {
		...
		tm_pid = pid;
		$.ajax({
			url: urlpath + '/user/practice/findPracticePaper.jhtml',
			type: "GET",
			async: false,
			success: function (t) {
				// console.log(t);
				...
				if (success) {
					...
					$('.progress_num i').html(questionCounts.length);
                    ...
                    // 计算试题容器的宽度
                    $('.div2').width(questionCounts.length*905);
					//启动翻题函数
	...
	savaQuestions: function () {
		...
		duration = duration - Math.floor(leftsecond/60);
	...
	ShowCountDown: function () {
		...
		if(leftsecond == 0){
			...
			layer.open({
				...
				closeBtn: 1,
				skin: 'layui-layer-molv',
				content: '<div style="text-align:center;">已到结束时间，是否提交？</div>',
				yes: function (index, layero) {
					layer.close(index);
					$(window).unbind('beforeunload');
					inTheTest.savaQuestions();
				}
			});
		}else{
			leftsecond --;
...
var tmUserDataCache = {
	...
	addCache: function (uid, pid, qid, qtype, val, rect) {
		...
		try {
			...
			sessionStorage.setItem(cacheKey, strCacheData);
