...
var editItemPage = {
	...
	init: function() {
		...
		tmPaper.uiInit();
		var id = editItemPage.getQueryString('id');
		//根据试卷id获取试卷内容
		$.ajax({
			...
			success: function(t) {
				...
				if(success) {
					...
					for(j = 0; j < datas.length; j++) {
						...
						contenthtml.push('	<div style="margin: 10px auto 10px 26px;">');
						...
						contenthtml.push('   <span>' + editItemPage.replaceInput(datas[j].qcontent) + '</span>');
						contenthtml.push('   <span class="span3" onclick="javascript:tmPaper.removeItem(this);"><i class="fa fa-close" style="float: right;margin-right: 15px;margin-top: 5px;color: #FE8922;font-size: 18px;"></i></span>');
