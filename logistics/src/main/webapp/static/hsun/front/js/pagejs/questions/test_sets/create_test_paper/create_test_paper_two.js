$(function() {
	createTestPaperTwo.init();
});
//获取岗位信息
var globalPost;
var createTestPaperTwo = {
	//初始化
	init: function() {
		...
		//							questionhtml.push('<span><a href="javascript:;" class="active" name="postName" onclick="choosePost(this,\'' + datas[i].value + '\')" value="' + datas[i].label + '">' + datas[i].label + '</a></span>');
		//						} else {
				...
				if(t.success) {
					layer.open({
						...
						yes: function(index, layero){
							layer.close(index);
							var id = datas.id;
							window.location.href = "../edit_item_page/edit_item_random_two.html?id=" + id+timestampv;
						}
					});
				} else {
					layer.open({
