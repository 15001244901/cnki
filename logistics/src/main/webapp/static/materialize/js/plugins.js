...
$(function() {
  ...
  * Plugin initialization
  ...
  //左侧菜单收起
  $('.toggle-leftslider').off('click').on('click',function () {
    if($('#slide-out').css('left') == '0px'){
      $('#slide-out').stop().animate({
        'left':'-250px'
      });
      $('#main').animate({
        'paddingLeft':'0px'
  ...
  $(window).resize(function(){
    ...
    //左侧菜单栏高度调整
    var left_slider_height = $(window).height() - 64;
    $('#slide-out').height(left_slider_height);
    $('#content').children('iframe').height(left_slider_height);
  });
