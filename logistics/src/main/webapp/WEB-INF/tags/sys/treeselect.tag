...
<%@ attribute name="showParentName" type="java.lang.Boolean" required="false" description="是否显示父节点名称，默认不显示"%>
				...
				if (v=="ok"){
					...
					for(var i=0; i<nodes.length; i++) {//<c:if test="${checked && notAllowSelectParent}">
						...
						//<c:if test="${showParentName}">
							_name = showParentName(nodes[i]);
						// 需要显示父节点名称</c:if>
						names.push(_name);//<c:if test="${!checked}">
