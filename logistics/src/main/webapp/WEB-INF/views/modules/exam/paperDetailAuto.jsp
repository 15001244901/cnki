		...
		var tmPaper = {
			uiInit : function(){
				...
				$( ".tm_adm_questionlist" ).bind('sortstop', function(event, ui) {
                    ...
                    //拖动后将当前项分值设置为段落设定的分值
                    var sectionScore =
                            ui["item"].closest(".tm_adm_paper_body_section_dl").find('input[name="p_scores"]').val();
                    ui["item"].find('.tm_qscore').val(sectionScore);
...
//				});
				...
				$("input[name='p_qnums']").each(function(i,o){
					...
					$(this).closest(".tm_adm_paper_body_section_dl").find(".tm_qscore").each(function(_i,_o){
						$(_o).val(_score);
					});
				...
				$.getJSON(surl, {pageNo:pageid, qType:q_type, qDbid:q_dbid, qLevel:q_level, qContent:q_content, t:Math.random()}, function(data){
					if(data){
						$.each(data["list"],function(idx, itm){
							...
							arrline.push('	分值 : ');
							arrline.push('	<input type="text" name="p_question_scores_" class="validate[required,custom[integer],min[1]] tm_qscore" value="0" size="1" readonly="readonly"/>');
							arrline.push('</td>');
						...
						<div class="tm_adm_paper">
							<div class="tm_adm_paper_head">
								...
								<h2 style="background:#ddd; padding:5px 0;">
									<b>智能组卷</b> &nbsp;
									根据每个段落的知识点、题型、难度、试题数量自动出题。<br/>如果某个段落有手动选题的情况则不再按照自动出题规则出题！
								</h2>
								<h2>
												...
												<dl class="tm_adm_paper_body_section_dl">
													...
													<dt>
														...
														<span class="tm_section_setting">
															<select name="p_dbids" class="tm_select" style="min-width:80px">
															...
															<select name="p_subjects" class="tm_select" style="min-width:100px">
															...
															<select name="p_qtypes" class="validate[required] tm_select" style="min-width:80px">
															...
															<select name="p_levels" class="tm_select" style="min-width:80px">
																...
																<c:forEach var="question" items="${section.questions}">
																	<li><table class="question_item">
																		<tr>
																			<td class="td1">
																				分值 :
																				<input type="text" name="p_question_scores_${section.id}" class="validate[required,custom[integer],min[1]] tm_qscore" value="${question.QScore}" size="1" readonly="readonly"/>
																			</td>
																			<td class="td2">
																					${question.QContent}
