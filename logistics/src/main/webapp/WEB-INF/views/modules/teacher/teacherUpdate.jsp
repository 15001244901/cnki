...
<div id="cont">
    <ul class="nav nav-tabs">
        ...
        <li class="active"><a href="${ctx}/teacher/toUpdate.jhtml">教师修改</a></li>
    </ul>
...
//        初始化加载数据
        function loadData(){
            var isid = getQueryString('id');
            $.ajax({
                'url':'${ctx}/teacher/'+isid+'/get.jhtml',
                'type':'get',
                'dataType': "json",
                'data':{
                    'id':isid
                ...
                success:function(ret){
                    //console.log(ret);
                    var data = ret.teacher;
                    $('input[name = name]').val(data.name);
                    $('[name = education]').val(data.education);
                    $('input[name = sort]').val(data.sort);
                    $('[name = isStar]').val(data.isStar);
                    if(ret.subject){
                        $('input[name = subjectId]').val(ret.subject.subjectId);
                        $('input[name = subjectName]').val(ret.subject.subjectName);
                    }
                    $('#logo').val(data.picPath);
                    if(data.picPath){
                        $('#logoPreview li:first').html('<img src="'+data.picPath+'" url="'+data.picPath+'" style="max-width:200px;max-height:200px;_height:200px;border:0;padding:3px;">&nbsp;&nbsp;<a href="javascript:" onclick="logoDel(this);">×</a>')
                    }
                    contextCkeditor.setData(data.career);
