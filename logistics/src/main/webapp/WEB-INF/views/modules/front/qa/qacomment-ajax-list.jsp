				...
				<ul class="pr10">
					<c:forEach items="${questions.questionsCommentList}" var="questionsComment">
						<li>
							<aside class="noter-pic">
								<c:choose>
									<c:when test="${not empty questionsComment.picImg}">
										<img src="${ctxStatic}${questionsComment.picImg}" alt="" width="50" height="50">
									</c:when>
									<c:otherwise>
										<img src="${ctxStatic}/hsun/front/qa/img/avatar-boy.gif" alt="" width="50" height="50">
									</c:otherwise>
								</c:choose>
							</aside>
							<div class="of hLh20">
							...
							<div class="of mt5">
								<span class="fr"><font class="fsize12 c-999 ml5"><fmt:formatDate type="both" value="${questionsComment.addTime }" pattern="yyyy-MM-dd HH:mm" /></font></span> <span class="fl good_zan"> <a href="javascript: void(0)" title="回答" class="noter-dy vam"
							...
							<div class="n-reply">
								<section class="n-reply-wrap mt10">
									<fieldset>
										<textarea name="" placeholder="输入您要评论的文字" id="commentReply${questionsComment.id}"></textarea>
									</fieldset>
									<p class="of mt5 tar pl10 pr10">
										<span class="fl "><tt class="c-red commentContentmeg" style="display: none;"></tt></span>
										<a href="javascript:;" onclick="addReply2(this,${questionsComment.id});" title="回复" class="lh-reply-btn">回复</a>
									</p>
								</section>
...
<div class="q-i-noter-box">
						...
						<aside class="noter-pic">
							<c:choose>
								<c:when test="${not empty user.photo }">
									<img src="${user.photo }" alt="" width="50" height="50">
								</c:when>
								<c:otherwise>
									<img src="${ctxStatic}/hsun/front/qa/img/avatar-boy.gif" alt="" width="50" height="50">
								</c:otherwise>
	...
	<c:if test="${empty questionsCommentList and empty questions.questionsCommentList }">
		<!-- /无数据提示 开始-->
	...
	<c:if test="${empty questionsCommentList and not empty questions.questionsCommentList }">
		<!-- /无数据提示 开始-->
		<section class="no-data-wrap">
			<em class="icon30 no-data-ico">&nbsp;</em> <span class="c-666 fsize14 ml10 vam">该问题已采纳最佳答案，您还可以对以上交流信息进行评论哦...</span>
		</section>
		<!-- /无数据提示 结束-->
	</c:if>
	<c:if test="${not empty questionsCommentList  }">
					...
					<c:forEach items="${questionsCommentList}" var="questionsComment" varStatus="index">
						<li>
							<aside class="noter-pic">
								<c:choose>
									<c:when test="${not empty questionsComment.picImg }">
										<img src="${questionsComment.picImg }" alt="" width="50" height="50">
									</c:when>
									<c:otherwise>
										<img src="${ctxStatic}/hsun/front/qa/img/avatar-boy.gif" alt="" width="50" height="50">
									</c:otherwise>
							...
							<div class="of hLh20">
								<c:if test="${questions.status==0 and  questions.cusId==user.id and questionsComment.cusId != questions.cusId}">
							...
							<div class="of mt5">
								<span class="fr"><font class="fsize12 c-999 ml5"> <!-- <a href="" class="c-blue mr10">删除</a> --> <fmt:formatDate type="both" value="${questionsComment.addTime}" pattern="yyyy-MM-dd HH:mm" />
									...
									<a class="p_chat vam ml10" data-id="${questionsComment.cusId}" href="javascript:void(0);"><em class="fa fa-commenting"></em> 私聊</a>
								</span>
							</div>
		...
		<!-- 公共分页 开始 -->
		<jsp:include page="/WEB-INF/views/modules/front/qa/ajaxpage.jsp"></jsp:include>
		<!-- 公共分页 结束 -->
</div>
<!-- /回答列表 结束 -->