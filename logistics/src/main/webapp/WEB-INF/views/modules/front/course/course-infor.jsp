...
<!--<![endif]-->
<head>
    <%@include file="course-base.jsp"%>
<script src="http://www.bfvyun.com/html/script/bfcloud.js?v=2"></script>
<script type="text/javascript">
	var isok = ${isok};
	var currentprice=${course.currentPrice};
</script>
    <title>地理云课堂-课程（${course.courseName}）详情</title>
</head>
<body>
	...
	<div id="aCoursesList" class="of">
		...
		<section class="container">
			<section class="path-wrap txtOf hLh30">
				...
				\
				<a href="${ctxRoot}/course/showcoulist" title="" class="c-999 fsize14">课程列表</a>
			...
			<div>
				<article class="c-v-pic-wrap">
					<section class="p-h-video-box" id="videoPlay">
						<c:choose>
							<c:when test="${course.logo!=null &&course.logo!=''}">
								<img src="${course.logo}" alt="${course.courseName}" class="dis c-v-pic hide" width="640" height="357"/>
							</c:when>
							<c:otherwise>
								<img src="${ctxStatic}/hsun/front/qa/img/default-img.gif" alt="${course.courseName}" class="dis c-v-pic hide" />
							</c:otherwise>
						</c:choose>
					...
					<section class="p-h-video-tip">
						<img src="${ctxStatic}/hsun/front/qa/img/v-loading.gif">
				...
				<aside class="c-attr-wrap">
					<section class="ml20 mr15">
						...
						<section class="c-attr-jg">
							...
							<span class="c-fff">简介：${course.title}</span>
						</section>
						...
						<section class="c-attr-mt">
									<a href="javascript:void(0)" title="立即观看" onclick="if(isLogin()){ window.location.href='${ctxRoot}/course/play/${course.courseId }'} else{lrFun();} " class="comm-btn c-btn-3">立即观看</a>
				...
				<aside class="thr-attr-box">
					<ol class="thr-attr-ol clearfix">
						<li><p>&nbsp;</p>
							<aside>
								<span class="c-fff f-fM">收藏数</span><br>
							...
							<section class="stud-act-list">
								<c:forEach items="${teacherList }" var="tea">
									<ul style="height: auto;">
										<li>
											<div class="u-face">
												<a href="${ctxRoot}/front/teacher/${tea.id }">
													<c:choose>
														<c:when test="${not empty tea.picPath }">
															<img src="${tea.picPath}" width="50" height="50" alt="">
														</c:when>
														<c:otherwise>
															<img src="${ctxStatic}/hsun/front/qa/img/avatar-boy.gif" width="50" height="50" alt="">
														</c:otherwise>
													</c:choose>
												</a>
											</div>
											<section class="hLh30 txtOf">
												<a class="c-333 fsize16 fl" href="${ctxRoot}/front/teacher/${tea.id }">${tea.name }</a>
											</section>
										...
										<c:forEach items="${courseList}" var="interfixCourse">
											<li>
												<aside class="course-r-pic">
													<a href="${ctxRoot}/course/couinfo/${interfixCourse.courseId}" title="">
														<c:choose>
															<c:when test="${interfixCourse.logo!=null && interfixCourse.logo!=''}">
																<img alt="" src="${interfixCourse.logo}" />
															</c:when>
															<c:otherwise>
																<img alt="" src="${ctxStatic}/hsun/front/qa/img/default-img.gif" />
															</c:otherwise>
														</c:choose>
													</a>
												</aside>
												<section class="hLh20 txtOf">
													<a href="${ctxRoot}/course/couinfo/${interfixCourse.courseId}" class="c-333 fsize16">${interfixCourse.courseName}</a>
												</section>
	...
	<script type="text/javascript" src="${ctxStatic}/hsun/front/qa/js/courseInfo.js"></script>
	<script type="text/javascript" src="${ctxStatic}/hsun/front/qa/js/comment.js"></script>
	<script>
		//评论课程id
		...
		$(function() {
			if(especial_public){
				...
				vedioClick(${freeVideoId});
				var is_video_width = $('.c-v-pic-wrap').width();
				$('.c-v-pic-wrap').css('height',Math.ceil((is_video_width*9)/16));
				$('#videoPlay').find('iframe').prop('scrolling','no');
